  ...
  implements IEventDriver
{
  ...
  batchAddEventListener(type: any, listener: any, options?: any) {
    ...
    engine[DRIVER_INSTANCES_SYMBOL] = engine[DRIVER_INSTANCES_SYMBOL] || [];
    if (!engine[DRIVER_INSTANCES_SYMBOL].includes(this)) {
      engine[DRIVER_INSTANCES_SYMBOL].push(this);
    }
    engine[DRIVER_INSTANCES_SYMBOL].forEach((driver: EventDriver) => {
      const target = driver.eventTarget(type) as any;
...
export class Event extends Subscribable<ICustomEvent<any>> {
  ...
  detachEvents(container?: EventContainer): EventDriver | undefined {
    if (!container) {
    ...
    if (!(container as any)[ATTACHED_SYMBOL]) return;
    (container as any)[ATTACHED_SYMBOL].forEach((driver: EventDriver) => {
      driver.detach(container);
    });

    (this as any)[DRIVER_INSTANCES_SYMBOL] =
      (this as any)[DRIVER_INSTANCES_SYMBOL] || [];
    (this as any)[DRIVER_INSTANCES_SYMBOL] = (this as any)[
      DRIVER_INSTANCES_SYMBOL
    ].reduce((drivers: EventDriver[], driver: EventDriver) => {
